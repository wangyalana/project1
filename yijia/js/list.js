...
//	边框变色
//  $(".list-wrap .list-ul .content_one").hover(function(){
//  	alert("ok");
...
//  });
	...
	$("#articleList").delegate(".content_one", "click", function(){
		//alert("article.html?"+"type=" + getUrlParams("type")+ "&articleId=" + $(this).attr("articleId"), "_blank");
		window.open("article.html?"+"type="+getUrlParams("type") + "&articleId=" + $(this).attr("articleId"), "_blank");//article.html?type=&articleId=1
	});
...
function loadArticleList(){
	...
	var itemHtml = '';
	var result = listData["listData0" +GLOBAL.pageStart]; //listData["listData01"] 此数据在listData.js里
	...
	if(GLOBAL.pageStart >= GLOBAL.pageCount){
		$(".no-img").attr("src","images/list_gomore_bgabb.jpg");
	}
}
